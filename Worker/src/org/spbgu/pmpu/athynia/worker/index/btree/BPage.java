...
public final class BPage implements Serializer {
    ...
    BPage(BTree btree, BPage root, BPage overflow)
        ...
        _recid = _btree._resman.insert(this, this);
    }
    ...
    InsertResult insert(int height, Object key, Object value, boolean replace)
        ...
        if (height == 0) {
            ...
            if (compare(key, _keys[index]) == 0) {
                ...
                if (replace) {
                    _values[index] = value;
                    _btree._resman.update(_recid, this, this);
                }
        ...
        if (_isLeaf) {
            ...
            if (_previous != 0) {
                ...
                _btree._resman.update(_previous, previous, this);
            }
            _previous = newPage._recid;
        }

        _btree._resman.update(_recid, this, this);
        _btree._resman.update(newPage._recid, newPage, this);
            ...
            if (result._underflow) {
                ...
                if (index < _children.length - 1) {
                    ...
                    if (bfirst < half) {
                        ...
                        // update BPages
                        _btree._resman.update(_recid, this, this);
                        _btree._resman.update(brother._recid, brother, this);
                        _btree._resman.update(child._recid, child, this);
                        ...
                        if (child._previous != 0) {
                            ...
                            _btree._resman.update(prev._recid, prev, this);
                        }
                        if (child._next != 0) {
                            BPage next = loadBPage(child._next);
                            next._previous = child._previous;
                            _btree._resman.update(next._recid, next, this);
                        }

                        // delete "child" BPage
                        _btree._resman.delete(child._recid);
                    }
                } else {
                    ...
                    if (bfirst < half) {
                        ...
                        _btree._resman.delete(brother._recid);
                    }
                }
            }
        }
...
*/
    ...
    private BPage loadBPage(long recid)
        throws IOException {
        BPage child = (BPage) _btree._resman.fetch(recid, this);
        child._recid = recid;
        child._btree = _btree;
        return child;
    }
